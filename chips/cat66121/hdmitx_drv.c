...
//        HDMITX_DEBUG_PRINTF(("getHDMITX_EDIDBytes(): Invallid pData pointer %08lX\n",(ULONG)pData));
    ...
    while(RemainedCount > 0)
    {
        ...
        HDMITX_WriteI2C_Byte(REG_TX_DDC_MASTER_CTRL,B_TX_MASTERDDC|B_TX_MASTERHOST); //0x10
        HDMITX_WriteI2C_Byte(REG_TX_DDC_HEADER,DDC_EDID_ADDRESS); // for EDID ucdata get 0x11
        HDMITX_WriteI2C_Byte(REG_TX_DDC_REQOFF,bCurrOffset); //0x12
        HDMITX_WriteI2C_Byte(REG_TX_DDC_REQCOUNT,(BYTE)ReqCount); //0x13
        HDMITX_WriteI2C_Byte(REG_TX_DDC_EDIDSEG,bSegment); //0x14
        HDMITX_WriteI2C_Byte(REG_TX_DDC_CMD,CMD_EDID_READ); //0x15
