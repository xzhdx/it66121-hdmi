...
void cat66121_hdmi_interrupt(struct hdmi *hdmi_drv)
{
	...
	sysstat = HDMITX_ReadI2C_Byte(REG_TX_SYS_STATUS); //0x0E
	if((sysstat & B_TX_INT_ACTIVE) || ((B_TX_HPDETECT & cat66121_hdmi->plug_status) != (B_TX_HPDETECT & sysstat)))  {
    		char intdata1,intdata2,intdata3;
		intdata1 = HDMITX_ReadI2C_Byte(REG_TX_INT_STAT1); //0x06
		intdata2 = HDMITX_ReadI2C_Byte(REG_TX_INT_STAT2); //0x07
		intdata3 = HDMITX_ReadI2C_Byte(REG_TX_INT_STAT3); //0x08
